function solve() {
   ...
   function onClick() {
      ...
      let data = JSON.
         parse(document.querySelector('#inputs textarea').value);

      let isBestRest = BestRestaurant(data);

      document.querySelector('#bestRestaurant>p')
      .textContent = getRest(isBestRest);
      document.querySelector('#workers>p')
      .textContent = getWorkers(isBestRest.workers);
   }

   function getRest(isBestRest) {
      return `Name: ${isBestRest.name} Average Salary: ${isBestRest.avgSalary.toFixed(2)} Best Salary: ${isBestRest.maxSalary.toFixed(2)}`;
  }
